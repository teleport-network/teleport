...
func (suite *TendermintTestSuite) TestExportMetadata() {
	...
	suite.chainA.App.XIBCKeeper.ClientKeeper.SetClientState(suite.chainA.GetContext(), "clientA", clientState)

	gm := clientState.ExportMetadata(suite.chainA.App.XIBCKeeper.ClientKeeper.ClientStore(suite.chainA.GetContext(), "clientA"))
	suite.Require().Nil(gm, "client with no metadata returned non-nil exported metadata")

	clientStore := suite.chainA.App.XIBCKeeper.ClientKeeper.ClientStore(suite.chainA.GetContext(), "clientA")
