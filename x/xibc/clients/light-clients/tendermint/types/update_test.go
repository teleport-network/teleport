...
import (
	...
	types "github.com/teleport-network/teleport/x/xibc/clients/light-clients/tendermint/types"
...
func (suite *TendermintTestSuite) TestCheckHeaderAndUpdateState() {
	...
	testCases := []struct {
		...
		setup: func(suite *TendermintTestSuite) {
			...
			consensusState = types.NewConsensusState(suite.clientTime, suite.header.Header.GetAppHash(), suite.valsHash)
		...
		setup: func(suite *TendermintTestSuite) {
			clientState = types.NewClientState(chainID, types.DefaultTrustLevel, trustingPeriod, ubdPeriod, maxClockDrift, height, commitmenttypes.GetSDKSpecs(), prefix, 0)
			consensusState = types.NewConsensusState(suite.clientTime, suite.header.Header.GetAppHash(), bothValSet.Hash())
		...
		setup: func(suite *TendermintTestSuite) {
			...
			suite.chainA.App.XIBCKeeper.ClientKeeper.SetClientConsensusState(ctx, chainName, heightPlus1, newHeader.ConsensusState())
		},
		expPass: true,
	}, {
	...
	for i, tc := range testCases {
		suite.Run(fmt.Sprintf("Case: %s", tc.name), func() {
			...
			suite.chainA.App.XIBCKeeper.ClientKeeper.SetClientConsensusState(ctx, chainName, consStateHeight, consensusState)

			height := newHeader.GetHeight()
			expectedConsensus := &types.ConsensusState{
				Timestamp:          newHeader.GetTime(),
				Root:               newHeader.Header.GetAppHash(),
			...
			newClientState, consensusState, err := clientState.CheckHeaderAndUpdateState(
				ctx,
				suite.cdc,
				suite.chainA.App.XIBCKeeper.ClientKeeper.ClientStore(suite.chainA.GetContext(), chainName), // pass in chainName prefixed clientStore
				newHeader,
			)
