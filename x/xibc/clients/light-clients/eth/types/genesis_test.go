...
func (suite *ETHTestSuite) TestExportMetadata() {
	...
	suite.Require().Equal(xibcethtypes.EthHeaderIndexKey(protoHeader.Hash(), protoHeader.Height.RevisionHeight), gm[0].GetKey(), "metadata has unexpected key")
	suite.Require().Equal(headerBytes, gm[0].GetValue(), "metadata has unexpected value")

	suite.Require().Equal(xibcethtypes.EthRootMainKey(common.BytesToHash(protoHeader.Root), protoHeader.Height.RevisionHeight), gm[1].GetKey(), "metadata has unexpected key")
