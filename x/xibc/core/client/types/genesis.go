...
// DefaultGenesisState returns the xibc client submodule's default genesis state.
...
func (gs GenesisState) Validate() error {
	...
	for _, clientMetadata := range gs.ClientsMetadata {
		...
		if _, ok := validClients[clientMetadata.ChainName]; !ok {
	...
	return host.ClientIdentifierValidator(gs.NativeChainName)
}
