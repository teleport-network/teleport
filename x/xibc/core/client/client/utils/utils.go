...
import (
	...
	xibcclient "github.com/teleport-network/teleport/x/xibc/client"
...
func QueryClientStateABCI(
	...
	value, proofBz, proofHeight, err := xibcclient.QueryTendermintProof(clientCtx, key)
	if err != nil {
		return nil, err
	}
...
// tendermint header
func QueryTendermintHeader(clientCtx client.Context) (xibctmtypes.Header, int64, error) {
	node, err := clientCtx.GetNode()
	if err != nil {
		return xibctmtypes.Header{}, 0, err
	}
...
func QueryNodeConsensusState(clientCtx client.Context) (*xibctmtypes.ConsensusState, int64, error) {
	node, err := clientCtx.GetNode()
	if err != nil {
		return &xibctmtypes.ConsensusState{}, 0, err
	}
	...
	state := &xibctmtypes.ConsensusState{
