...
func NewCreateClientProposalCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "create a new XIBC client with the specified client state and consensus state",
...
// NewUpdateClientCmd defines the command to update an XIBC client.
func NewUpdateClientCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "update [chain-name] [path/to/header.json]",
		Short:   "update existing client with a header",
		Long:    "update existing client with a header",
		Example: fmt.Sprintf("%s tx xibc %s update [chain-name] [path/to/header.json] --from node0 --home ../node0/<app>cli --chain-id $CID", version.AppName, types.SubModuleName),
...
func NewUpgradeClientProposalCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "upgrade a XIBC client with the specified client state and consensus state",
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
				return err
