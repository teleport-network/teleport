...
func (suite *KeeperTestSuite) TestQueryPacketCommitment() {
	...
	testCases := []struct {
		...
		func() {
			...
			req = &types.QueryPacketCommitmentRequest{
				SrcChain: path.EndpointA.ChainName,
				DstChain: path.EndpointB.ChainName,
				Sequence: 1,
			}
		},
		true,
	...
	testCases := []struct {
		...
		func() {
			...
			for i := uint64(0); i < 9; i++ {
				...
				suite.chainA.App.XIBCKeeper.PacketKeeper.SetPacketCommitment(suite.chainA.GetContext(), path.EndpointA.ChainName, path.EndpointB.ChainName, commitment.Sequence, commitment.Data)
...
func (suite *KeeperTestSuite) TestQueryPacketReceipt() {
	...
	testCases := []struct {
		...
		func() {
			path := xibctesting.NewPath(suite.chainA, suite.chainB)
			suite.coordinator.SetupClients(path)
			suite.chainA.App.XIBCKeeper.PacketKeeper.SetPacketReceipt(suite.chainA.GetContext(), path.EndpointA.ChainName, path.EndpointB.ChainName, 1)
...
func (suite *KeeperTestSuite) TestQueryPacketAcknowledgement() {
	...
	testCases := []struct {
		...
		func() {
			...
			expAck = []byte("hash")
			suite.chainA.App.XIBCKeeper.PacketKeeper.SetPacketAcknowledgement(suite.chainA.GetContext(), path.EndpointA.ChainName, path.EndpointB.ChainName, 1, expAck)
	...
	testCases := []struct {
		...
		func() {
			...
			for i := uint64(0); i < 9; i++ {
				...
				suite.chainA.App.XIBCKeeper.PacketKeeper.SetPacketAcknowledgement(suite.chainA.GetContext(), path.EndpointA.ChainName, path.EndpointB.ChainName, ack.Sequence, ack.Data)
	...
	testCases := []struct {
		...
		func() {
			...
			for seq := uint64(1); seq < 10; seq++ {
				...
				if seq%2 == 0 {
					suite.chainA.App.XIBCKeeper.PacketKeeper.SetPacketReceipt(suite.chainA.GetContext(), path.EndpointA.ChainName, path.EndpointB.ChainName, seq)
				} else {
...
func (suite *KeeperTestSuite) TestQueryUnreceivedAcks() {
	...
	testCases := []struct {
		...
		func() {
			...
			suite.chainA.App.XIBCKeeper.PacketKeeper.SetPacketCommitment(suite.chainA.GetContext(), path.EndpointA.ChainName, path.EndpointB.ChainName, 1, []byte("commitment"))
		...
		func() {
			...
			for seq := uint64(1); seq < 10; seq++ {
				...
				if seq%2 == 0 {
					suite.chainA.App.XIBCKeeper.PacketKeeper.SetPacketCommitment(suite.chainA.GetContext(), path.EndpointA.ChainName, path.EndpointB.ChainName, seq, []byte("commitement"))
					expSeq = append(expSeq, seq)
				}
			}
