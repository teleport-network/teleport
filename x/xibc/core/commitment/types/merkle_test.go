...
import (
	...
	"github.com/teleport-network/teleport/x/xibc/core/commitment/types"
)
...
func (suite *MerkleTestSuite) TestVerifyMembership() {
	...
	cases := []struct {
		...
		{"nil root", []byte(nil), []string{suite.storeKey.Name(), "MYKEY"}, []byte("MYVALUE"), func() {}, false},           // invalid proof with nil root
		{
			...
			[]byte("MYVALUE"),
			func() { proof = types.MerkleProof{Proofs: proof.Proofs[1:]} },
			false,
	...
	for i, tc := range cases {
		suite.Run(tc.name, func() {
			...
			err := proof.VerifyMembership(types.GetSDKSpecs(), tc.root, path, tc.value)
			if tc.shouldPass {
				// nolint: scopelint
...
func (suite *MerkleTestSuite) TestVerifyNonMembership() {
	...
	for i, tc := range cases {
		suite.Run(tc.name, func() {
			...
			err := proof.VerifyNonMembership(types.GetSDKSpecs(), tc.root, path)
			if tc.shouldPass {
				// nolint: scopelint
				suite.Require().NoError(err, "test case %d should have passed", i)
			} else {
