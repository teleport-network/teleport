...
func (proof MerkleProof) VerifyMembership(specs []*ics23.ProofSpec, root []byte, path exported.Path, value []byte) error {
	...
	if err := verifyChainedMembershipProof(root, specs, proof.Proofs, mpath, value, 0); err != nil {
		return err
	}
	return nil
}
...
func (proof MerkleProof) VerifyNonMembership(specs []*ics23.ProofSpec, root []byte, path exported.Path) error {
	if err := proof.validateVerificationArgs(specs, root); err != nil {
		return err
	}
	...
	case *ics23.CommitmentProof_Nonexist:
		...
		if err := verifyChainedMembershipProof(root, specs, proof.Proofs, mpath, subroot, 1); err != nil {
			return err
		}
...
func (proof MerkleProof) BatchVerifyMembership(specs []*ics23.ProofSpec, root []byte, path exported.Path, items map[string][]byte) error {
...
// NOTE: Currently left unimplemented as it is unused
func (proof MerkleProof) BatchVerifyNonMembership(specs []*ics23.ProofSpec, root []byte, path exported.Path, items [][]byte) error {
	return sdkerrors.Wrap(ErrInvalidProof, "batch proofs are currently unsupported")
}
...
func (proof MerkleProof) validateVerificationArgs(specs []*ics23.ProofSpec, root []byte) error {
	if proof.Empty() {
