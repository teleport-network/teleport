...
func (suite *KeeperTestSuite) SendTransferERC20(sender common.Address, data types.CallRCCData) {
	res, err := suite.app.XIBCRCCKeeper.CallEVM(
		suite.ctx,
		rcc.RCCContract.ABI,
		sender,
		rcc.RCCContractAddress,
		"sendRemoteContractCall",
		data,
	)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) RecvPacket(data types.RCCPacketData) {
	res, err := suite.app.XIBCRCCKeeper.RecvPacket(suite.ctx, data)
	suite.Require().NoError(err)
	var r struct{ Result packettypes.Result }
	err = rcc.RCCContract.ABI.UnpackIntoInterface(&r, "onRecvPacket", res.Ret)
	suite.Require().NoError(err)
	suite.Require().Equal("", r.Result.Message)
	suite.Require().Equal([]byte{01}, r.Result.Result)
}

func (suite *KeeperTestSuite) AcknowledgementPacket(dataHash [32]byte, result []byte) {
	res, err := suite.app.XIBCRCCKeeper.AcknowledgementPacket(suite.ctx, dataHash, result)
	suite.Require().NoError(err)
	suite.Require().False(res.Failed(), res.VmError)
}
