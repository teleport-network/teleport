...
// ensureHooksSet tries to set the hooks on EVMKeeper, this will fail if aggregate hook is already set
func (suite *KeeperTestSuite) ensureHooksSet() {
	...
	suite.app.EvmKeeper.SetHooks(suite.app.AggregateKeeper.Hooks())
}
...
func (suite *KeeperTestSuite) TestEvmHooksRegisterCoin() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			_, err := suite.app.AggregateKeeper.ConvertCoin(ctx, convertCoin)
