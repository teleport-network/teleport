...
func (suite *KeeperTestSuite) TestGetTokenPair() {
	...
	for _, tc := range testCases {
		p, found := suite.app.AggregateKeeper.GetTokenPair(suite.ctx, tc.id)
		if tc.ok {
...
func (suite *KeeperTestSuite) TestIsTokenPairRegistered() {
	...
	for _, tc := range testCases {
		found := suite.app.AggregateKeeper.IsTokenPairRegistered(suite.ctx, tc.id)
		if tc.ok {
			suite.Require().True(found, tc.name)
		} else {
			suite.Require().False(found, tc.name)
...
func (suite *KeeperTestSuite) TestIsERC20Registered() {
	...
	for _, tc := range testCases {
		...
		found := suite.app.AggregateKeeper.IsERC20Registered(suite.ctx, tc.erc20)
