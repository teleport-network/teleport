...
func (suite *ProposalTestSuite) TestKeysTypes() {
	suite.Require().Equal("aggregate", (&RegisterCoinProposal{}).ProposalRoute())
	suite.Require().Equal("RegisterCoin", (&RegisterCoinProposal{}).ProposalType())
	suite.Require().Equal("aggregate", (&RegisterERC20Proposal{}).ProposalRoute())
	suite.Require().Equal("RegisterERC20", (&RegisterERC20Proposal{}).ProposalType())
	suite.Require().Equal("aggregate", (&UpdateTokenPairERC20Proposal{}).ProposalRoute())
	suite.Require().Equal("UpdateTokenPairERC20", (&UpdateTokenPairERC20Proposal{}).ProposalType())
	suite.Require().Equal("aggregate", (&ToggleTokenRelayProposal{}).ProposalRoute())
	suite.Require().Equal("ToggleTokenRelay", (&ToggleTokenRelayProposal{}).ProposalType())
}

func (suite *ProposalTestSuite) TestValidateAggregateDenom() {
	testCases := []struct {
		name    string
		denom   string
		expPass bool
		...
		{
			"- instead of /",
			"aggregate-0xdac17f958d2ee523a2206206994597c13d831ec7",
			false,
		},
		{
			"without /",
		...
		{
			"multiple /",
			"aggregate/0xdac17f958d2ee523a2206206994597c13d831ec7/test",
			false,
		},
		{
			"pass",
	...
	for _, tc := range testCases {
		err := ValidateAggregateDenom(tc.denom)
...
func (suite *ProposalTestSuite) TestUpdateTokenPairERC20ProposalGetERC20Addresses() {
	...
	suite.Require().Equal(addr, proposal.ConvertERC20Address())
	suite.Require().Equal(addrNew, proposal.ConvertNewERC20Address())
}
